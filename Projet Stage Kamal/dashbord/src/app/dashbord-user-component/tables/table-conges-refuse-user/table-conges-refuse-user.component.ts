...
@Component({
  selector: 'app-table-conges-refuse-user',
  templateUrl: './table-conges-refuse-user.component.html',
  styleUrls: ['./table-conges-refuse-user.component.css'],
})
export class TableCongesRefuseUserComponent {
  durationInSeconds = 5;
  userId!: string;
  ...
  ngOnInit() {
    ...
    this.http
      .get<CongeResponse>(
        'http://localhost:3800/api/table/gettablecongerefuseuser',
        {
          headers: {
  ...
  ExtraireFichierCSV() {
    ...
    this.csv.downloadCSV(strCsv, 'conges refuse user.csv');
  }
}
