...
@Component({
  selector: 'app-table-conges-accepte-user',
  templateUrl: './table-conges-accepte-user.component.html',
  styleUrls: ['./table-conges-accepte-user.component.css'],
})
export class TableCongesAccepteUserComponent {
  durationInSeconds = 5;
  userId!: string;
  ...
  ngOnInit() {
    ...
    this.http
      .get<CongeResponse>(
        'http://localhost:3800/api/table/gettablecongeacceptuser',
        {
          headers: {
  ...
  ExtraireFichierCSV() {
    ...
    this.csv.downloadCSV(strCsv, 'conges accepte user.csv');
  }
}
