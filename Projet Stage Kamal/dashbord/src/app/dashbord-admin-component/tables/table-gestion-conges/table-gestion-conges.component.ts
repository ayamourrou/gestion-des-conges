...
@Component({
  selector: 'app-table-gestion-conges',
  templateUrl: './table-gestion-conges.component.html',
  styleUrls: ['./table-gestion-conges.component.css'],
})
export class TableGestionCongesComponent implements OnInit, AfterViewInit {
  durationInSeconds = 5;
  ...
  ngOnInit(): void {
    this.http
      .get<CongeResponse>(
        'http://localhost:3800/api/table/gettablecongeresponse'
      )
      .subscribe((data) => {
        if (data.status == 200) {
  ...
  ExtraireFichierCSV() {
    ...
    this.csv.downloadCSV(strCsv, 'Getion des Conges Response.csv');
  }
}
