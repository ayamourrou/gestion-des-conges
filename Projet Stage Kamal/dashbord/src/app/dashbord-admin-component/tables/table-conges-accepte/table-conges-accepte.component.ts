...
@Component({
  selector: 'app-table-conges-accepte',
  templateUrl: './table-conges-accepte.component.html',
  styleUrls: ['./table-conges-accepte.component.css'],
})
export class TableCongesAccepteComponent implements OnInit, AfterViewInit {
  durationInSeconds = 5;
  ...
  ngOnInit(): void {
    this.http
      .get<CongeResponse>('http://localhost:3800/api/table/gettablecongeaccept')
      .subscribe((data) => {
        if (data.status == 200) {
  ...
  ExtraireFichierCSV() {
    ...
    this.csv.downloadCSV(strCsv, 'Getion des Conges Accepte.csv');
  }
}
