...
@Component({
  selector: 'app-table-conges-waiting',
  templateUrl: './table-conges-waiting.component.html',
  styleUrls: ['./table-conges-waiting.component.css'],
})
export class TableCongesWaitingComponent implements OnInit, AfterViewInit {
  durationInSeconds = 5;
  ...
  ngOnInit(): void {
    this.http
      .get<CongeResponse>(
        'http://localhost:3800/api/table/gettablecongewaiting'
      )
      .subscribe((data) => {
        if (data.status == 200) {
  ...
  ExtraireFichierCSV() {
    ...
    this.csv.downloadCSV(strCsv, 'Getion des Conges Waiting.csv');
  }
}
